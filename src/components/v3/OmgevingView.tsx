...
import { ChevronRight, ChevronDown, Download, Info } from 'lucide-react';
...
export const OmgevingView: React.FC<OmgevingViewProps> = ({
  ...
  return (
    <div className="flex flex-col min-h-screen pb-24 text-[var(--foreground)] bg-[var(--background)] selection:bg-[#2d2d3a]/10">
      {/* Top Bar */}
      <div className="px-5 pt-4 pb-2 flex items-center justify-between">
        <div className="flex flex-col">
          <span className="text-[8.5px] font-bold tracking-[0.2em] uppercase text-[var(--muted-foreground)]">
            Uitvaartzorg De Vijverhof
          </span>
          <h1 className="text-[19px] font-bold text-[var(--foreground)] tracking-tight m-0">
            Uw dossiers
          </h1>
        </div>
        ...
        <button
          ...
          className="w-8 h-8 rounded-full border border-[var(--border-strong)] flex items-center justify-center text-[var(--muted-foreground)] hover:text-[var(--foreground)] hover:bg-[var(--secondary)] transition-colors cursor-pointer"
        >
          <Info size={16} />
        </button>
      </div>
      ...
      {/* Info Popover */}
      {infoOpen && (
        <div className="mx-5 my-2 p-4 rounded-2xl bg-[var(--glass-nav-bg)] border border-[var(--glass-nav-border)] backdrop-blur-xl shadow-lg flex flex-col gap-2 animate-fadeIn z-50">
          <div className="flex items-center justify-between">
            <span className="text-[9.5px] font-bold uppercase tracking-widest text-[var(--muted-foreground)]">
              Over de omgeving
            </span>
            <button
              type="button"
          ...
          <p className="text-[13px] leading-relaxed text-[var(--foreground)] m-0 font-light">
            Eén strip onder de titel bepaalt wat u ziet. <strong>Aan u</strong> draagt een aantal: daar bent u zelf aan zet. <strong>Open</strong> is alles wat nog loopt. <strong>Alles</strong> bevat ook afgeronde dossiers.
          </p>
        </div>
      )}

      {/* WeergaveStrip (Aan u, Open, Alles) */}
      <div className="px-5 py-2">
        <div className="flex gap-1 bg-[var(--secondary)] rounded-full p-1 border border-[var(--border)]">
          <button
            type="button"
            onClick={() => setActiveTab('aanu')}
            className={`flex-1 py-1.5 px-3 rounded-full text-[12px] font-medium flex items-center justify-center gap-1.5 transition-all cursor-pointer ${
              activeTab === 'aanu'
                ? 'bg-[var(--background)] text-[var(--foreground)] font-bold shadow-xs'
                : 'text-[var(--muted-foreground)] hover:text-[var(--foreground)]'
            }`}
          >
            <span className="w-1.5 h-1.5 rounded-full bg-[var(--primary)] animate-pulse flex-none" />
            <span>Aan u</span>
            <span className="text-[10px] px-1.5 py-0.2 rounded-full bg-[var(--muted)] text-[var(--muted-foreground)] font-bold">
              {aanUCount}
            </span>
          </button>
      ...
      {/* Begeleider Filter Chips */}
      {activeTab !== 'aanu' && (
        <div className="px-5 py-2 flex items-center gap-2 overflow-x-auto mem-scroll">
          <span className="text-[9.5px] font-bold uppercase tracking-widest text-[var(--muted-foreground)] mr-1 flex-none">
            Begeleider
          </span>
          {[
            ...
            <button
              ...
              className={`px-3 py-1 rounded-full text-[11.5px] whitespace-nowrap transition-all cursor-pointer ${
                selectedCrew === c.id
                  ...
                  : 'border border-[var(--border-strong)] bg-transparent text-[var(--muted-foreground)] hover:text-[var(--foreground)]'
              }`}
            >
              {c.label}
            </button>
      ...
      <div className="px-5 pt-2 flex flex-col gap-3">
        {filteredDossiers.map((p) => {
          ...
          return (
            <div
              key={p.id}
              className={`rounded-[26px] p-4 flex flex-col transition-all duration-300 ${
                isAanU
                  ? 'bg-[var(--background)] border border-[var(--border-strong)] shadow-[0_4px_20px_-5px_rgba(0,0,0,0.05)]'
            ...
            >
              ...
              >
                {/* Thumbnail: 46x61 with radius 14px */}
                <div className="w-[46px] h-[61px] rounded-[14px] overflow-hidden bg-[var(--secondary)] border border-[var(--border)] flex-none relative shadow-2xs flex items-center justify-center">
                  {p.hasPortrait ? (
                    <img
                      src={p.photoFull}
                      alt={p.call}
                    ...
                    <span className="text-[11px] font-bold tracking-widest text-[var(--muted-foreground)]">
                      {p.call[0]}
                    </span>
                  )}
                </div>
                ...
                <div className="flex flex-col flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-[15.5px] font-bold text-[var(--foreground)] truncate">
                      {p.who}
                    </span>

                    {/* Status Badge */}
                    <span
                      className={`text-[8.5px] font-bold tracking-[0.16em] uppercase px-2.5 py-1 rounded-full flex-none flex items-center gap-1.5 ${
                        isAanU
                          ? 'bg-[var(--primary)] text-[var(--primary-foreground)]'
                          : 'border border-[var(--border-strong)] text-[var(--muted-foreground)] bg-transparent'
                      }`}
                    >
                      {isAanU && <span className="w-1.5 h-1.5 rounded-full bg-white animate-pulse" />}
                      <span>{isAanU ? 'Aan u' : `Bij ${p.begeleider}`}</span>
                    </span>
                  </div>

                  <span className="text-[12.5px] font-medium text-[var(--foreground)] mt-1">
                    {p.status}
                  </span>
                  <span className="text-[12px] text-[var(--muted-foreground)] leading-snug mt-0.5">
                    → {p.nextStep}
                  </span>
              ...
              {/* Melding van de familie (Neutral Secondary Box - NO YELLOW) */}
              {p.quote && (
                <div className="mt-3 p-3.5 rounded-[20px] bg-[var(--secondary)] border border-[var(--border)] flex flex-col gap-2 text-[12.5px]">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="w-1.5 h-1.5 rounded-full bg-[var(--primary)] animate-pulse flex-none" />
                      <span className="text-[9.5px] font-bold tracking-[0.2em] uppercase text-[var(--muted-foreground)]">
                        Melding van de familie
                      </span>
                    </div>
                    <span className="text-[11px] text-[var(--muted-foreground)]">{p.quoteTime}</span>
                  </div>
                  <p className="text-[13.5px] font-light leading-relaxed text-[var(--foreground)] m-0">
                    "{p.quote}"
                  </p>
              ...
              <div className="flex items-center gap-2 mt-3 pt-1">
                {p.id === 'greet' && (
                  <>
                    <button
                      ...
                      className="min-h-[38px] px-4 rounded-full bg-[var(--primary)] text-[var(--primary-foreground)] text-[12px] font-bold hover:opacity-90 transition-opacity cursor-pointer shadow-xs"
                    >
                      Gegeven corrigeren
                    </button>
                    ...
                    <button
                      ...
                      className="min-h-[38px] px-3.5 rounded-full bg-[var(--background)] border border-[var(--border-strong)] text-[var(--foreground)] text-[11.5px] font-medium hover:bg-[var(--secondary)] transition-colors cursor-pointer flex items-center gap-1.5"
                    >
                      <Download size={13} />
                ...
                {p.id === 'carien' && (
                  <>
                    ...
                    <button
                      ...
                      className="min-h-[38px] px-3.5 rounded-full bg-[var(--background)] border border-[var(--border-strong)] text-[var(--foreground)] text-[11.5px] font-medium hover:bg-[var(--secondary)] transition-colors cursor-pointer"
                    >
                      Herinnering sturen
                    </button>
                  </>
                ...
                <button
                  ...
                  className="w-[38px] h-[38px] rounded-full border border-[var(--border-strong)] bg-[var(--background)] flex items-center justify-center text-[var(--foreground)] hover:bg-[var(--secondary)] transition-colors cursor-pointer ml-auto flex-none"
      ...
      <div className="mt-8 px-5">
        <button
          ...
          className="w-full py-3 px-4 rounded-[22px] bg-[var(--secondary)] border border-[var(--border)] flex items-center justify-between text-left cursor-pointer hover:bg-[var(--muted)] transition-colors"
        >
          <div className="flex flex-col">
        ...
        {archiefOpen && (
          <div className="mt-2 p-3 rounded-[22px] bg-[var(--card)] border border-[var(--border)] flex flex-col gap-1.5 animate-fadeIn text-[12px]">
            {[
      ...
      {correctieOpen && (
        <div className="fixed inset-0 z-[120] bg-black/40 flex items-end justify-center backdrop-blur-xs animate-fadeIn">
          <div className="w-full max-w-[480px] bg-[var(--background)] rounded-t-[32px] p-6 shadow-2xl flex flex-col gap-4 border-t border-[var(--border-strong)] animate-slideUp">
            <div className="flex items-center justify-between">
              <div>
                <span className="text-[9.5px] font-bold tracking-widest uppercase text-[var(--muted-foreground)]">
            ...
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                className="w-full h-11 px-3.5 rounded-xl bg-[var(--input-background)] border border-[var(--border-strong)] text-[14px] text-[var(--foreground)] focus:outline-none focus:ring-1 focus:ring-[var(--primary)]"
              />
              <span className="text-[10.5px] text-[var(--muted-foreground)] text-right">
                {correctieVal.length} van 34 tekens
              </span>
            </div>

            <div className="p-3 rounded-xl bg-[var(--secondary)] text-[11.5px] text-[var(--muted-foreground)] leading-relaxed">
            ...
            <button
              ...
              className="w-full h-12 rounded-full bg-[var(--primary)] text-[var(--primary-foreground)] font-bold text-[13px] hover:opacity-90 transition-opacity cursor-pointer shadow-md mt-1"
            >
              Corrigeren & opslaan
            </button>
          </div>
